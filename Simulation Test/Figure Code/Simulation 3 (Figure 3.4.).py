...
# Load the datasets
file_path_1 = "D:/桌面/Thesis project/Data_analysis (Include Table, Figure)/Data-test/2.3 Description to Matrix/Description to Matrix (Simulation 2).csv"
file_path_2 = "D:/桌面/Thesis project/Data_analysis (Include Table, Figure)/Data-test/2.3 Description to Matrix/Description to Matrix (Simulation 12 compare).csv"
...
accuracy_api = data_2['Accuracy (Simulation 1)'].str.rstrip('%').astype(float) / 100.0
accuracy_web = data_2['Accuracy (Simulation 2)'].str.rstrip('%').astype(float) / 100.0
...
axes[0].set_title('Accuracy of the Taxonomy task in Simulation 3', fontsize=14, pad=20)
...
axes[1].plot(
    ...
    linestyle='-', marker='o', label='Simulation 1: Accuracy', zorder=1
)
axes[1].scatter(
    character_numbers_2, accuracy_api, color=styles['Comparison']['api_point_color'], label='Simulation 1: Mean Points', zorder=2
)
axes[1].plot(
    character_numbers_2, accuracy_web, color=styles['Comparison']['web_line_color'],
    linestyle='-', marker='o', label='Simulation 2: Accuracy', zorder=1
)
axes[1].scatter(
    character_numbers_2, accuracy_web, color=styles['Comparison']['web_point_color'], label='Simulation 2: Mean Points', zorder=2
)
...
axes[1].set_title('Accuracy Comparison Between Simulation 2 and Simulation 3', fontsize=14, pad=20)
...
plt.tight_layout()
plt.savefig("D:/桌面/Thesis project/Data_analysis (Include Table, Figure)/Figure-Chapter 2/SVG/Figure 2.6.png", format='png', dpi=600)
plt.show()
