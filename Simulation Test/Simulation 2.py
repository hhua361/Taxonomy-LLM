...
number_about_the_character = 15  # Define the number of characters to be generated
...
def process_species(matrix, character_list):
    ...
    for species_matrix in matrix:
        # Convert the species matrix to a knowledge graph
...
# Compare with the different matrix information, to validate the accuracy the
        ...
        if species_name in extracted_matrix_results:
            ...
            for i, (initial_value, extracted_value) in enumerate(zip(initial_matrix_values, extracted_matrix_values)):
                if initial_value != extracted_value:
                    differences.append(f"Character {i + 1}: Initial='{initial_value}' vs Extracted='{extracted_value}'")  # Recording of discrepancies
